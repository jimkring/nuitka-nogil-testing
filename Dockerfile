...
RUN apt-get update && apt-get install -y apt-utils && apt-get install -y patchelf && apt-get install python3-dev

WORKDIR /usr/src/app

COPY requirements.txt ./
#RUN pip install -U --force-reinstall "https://github.com/jimkring/Nuitka/archive/factory.zip"
RUN pip install git+https://github.com/Nuitka/Nuitka.git@factory
